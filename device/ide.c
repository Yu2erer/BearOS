...
// device 寄存器
#define BIT_DEV_MBS 0xa0 // 第7位和第5位固定为0
#define BIT_DEV_LBA 0x40
#define BIT_DEV_DEV 0x10

// 硬盘操作
#define CMD_IDENTIFY 0xec // identify 指令 获取硬盘身份信息
#define CMD_READ_SECTOR 0x20 // 读扇区
#define CMD_WRITE_SECTOR 0x30 // 写扇区

#define max_lba ((80*1024*1024 / 512) - 1) // 最大读取扇区数 80 MB 调试用
...
} __attribute__ ((packed)); // 按一字节对齐 紧凑 保证结构为16字节

// 引导扇区 MBR EBR 
struct boot_sector {
    uint8_t other[446]; // 引导
...
void intr_hd_handler(uint8_t irq_no) {
    ...
    if (channel->expecting_intr) {
        ...
        // 读取状态寄存器 使硬盘控制器认为此次中断已被处理 硬盘继续执行新的读写
        inb(reg_status(channel));
    }
}
// 将 dst 中len个相邻字节交换位置存入buf
// 硬盘参数信息都是字为单位 需要进行转换
    ...
    while (channel_no < channel_cnt) {
        ...
        switch (channel_no) {
            case 0:
                ...
                channel->irq_no = 0x20 + 14; // 从片 IRQ14引脚
                break;
            case 1:
