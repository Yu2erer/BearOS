...
struct task_struct* idle_thread; // idel 线程

// 系统空闲时 运行的线程
static void idle(void* arg UNUSED) {
    while (1) {
        thread_block(TASK_BLOCKED);
        asm volatile("sti; hlt": : : "memory"); // 执行hlt挂起必须先开中断
    }    
}
...
void init_thread(struct task_struct* pthread, char* name, int prio) {
    ...
    pthread->stack_magic = 0x66666666; // 自定义数 作为标记防止栈溢出
}
...
void schedule() {
    ...
    }  // 如果不是因为时间片到期而被换下 说明是被阻塞了 不在就绪队列
