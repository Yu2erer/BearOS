...
void pfree(uint32_t pg_phy_addr) {
    struct pool* mem_pool;
    uint32_t bit_idx = 0;
    if (pg_phy_addr >= user_pool.phy_addr_start) { // 用户物理内存池在内核物理内存池的上面
        mem_pool = &user_pool;
...
// 去掉页表虚拟地址vaddr映射 只去掉 vaddr 的 pte
...
void sys_free(void* ptr) {
    ...
    if (ptr != NULL) {
        ...
        if (a->desc == NULL && a->large == true) {
            ...
            list_append(&a->desc->free_list, &b->free_elem);
            if (++a->cnt == a->desc->blocks_per_arena) { // 都是空闲的 释放整页
                uint32_t block_idx;
